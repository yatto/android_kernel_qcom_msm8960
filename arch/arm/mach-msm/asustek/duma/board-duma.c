...
// ---- end of removed commeted area form duma

// remove commented area for duma
...
//end of removed commented area
//remove commeted area for DUMA
...
static struct msm_spm_platform_data msm_spm_l2_data[] __initdata = {
	[0] = {
		...
		.reg_init_values[MSM_SPM_REG_SAW2_PMIC_DLY] = 0x02020207,
...
#define TS_POWER_EN_GPIO 7
#define TS_RESET_GPIO 31
#define TS_TOUCH_ADDRESS 0x5C
...
static void touch_init(void){
	...
	if(machine_is_apq8064_duma()){
                    //set gpio
                    gpio_request(TS_POWER_EN_GPIO, "tp_power");
                    gpio_direction_output(TS_POWER_EN_GPIO, 1);
                    gpio_request(TS_RESET_GPIO, "tp_reset");
                    gpio_direction_output(TS_RESET_GPIO, 1);
                    gpio_request(TS_IRQ_GPIO,"irq");
                    gpio_direction_input(TS_IRQ_GPIO);
                    //[ELAN] Touch dirver register
                    platform = (struct elan_ktf3k_i2c_platform_data *)elan_i2c_devices[0].platform_data;
                    platform->abs_x_max = ELAN_X_MAX_302KL - 1;
                    platform->abs_y_max = ELAN_Y_MAX_302KL - 1;
                ...
                else{/* if(machine_is_apq8064_seamullet()||machine_is_apq8064_greymullet())*/
                    	// get LDO17
...
/*
	...
	sensor_platform_init(); // probabilmente da rimuovere...
...
int __init asustek_add_lid(void)
{
	...
	case HW_REV_E:
		...
		else if(machine_is_apq8064_deb() || machine_is_apq8064_duma())
...
{
	int rc = 0; /* commented because DUMA non have INPUT_LID. If you enable LID yaou have to uncomment this line*/
	printk(KERN_NOTICE "MIDR      = 0x%08x\n", read_cpuid_id());
	if (meminfo_init(SYS_MEMORY, SZ_256M) < 0)
		pr_err("meminfo_init() failed!\n");
	if (machine_is_apq8064_duma() &&  SOCINFO_VERSION_MINOR(socinfo_get_platform_version()) == 1)
